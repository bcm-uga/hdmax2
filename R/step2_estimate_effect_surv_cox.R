...
estimate_effect_surv_cox <- function(object , m, boots = 1000, ...) {
  ...
  for(expo_var_id in expo_var_ids){
    ...
    if (expo_var_type == "character"||is.factor(exposure)){
      ...
      for (k in 1:length(cn)) {
        ...
        ## faire un bootstrap pour chaque categorie pour estimer l'effet medié par contre je ne sais pas comment estimer les autres effets...
        
        ME <- matrix(ncol = 1, nrow = ncol(M))
        ...
        for (i in 1:ncol(M)) {#numeric
            ...
            base = survival::Surv(survival_time, censure)
            mod2 = survival::coxph(base ~ exposure_k + Mi + ., data = dat.y)
            
            mediator_effect <- coef(mediator_model)["exposure_k"]
            direct_effect <- coef(outcome_model)["exposure_k"]
            indirect_effect <- coef(outcome_model)["Mi"] * mediator_effect
