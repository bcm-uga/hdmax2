...
plot_V2 <- function(object, N_med = 10) {
  ...
  if (object$multivariate) {
    for(cat in 1:dim(object$X_matrix)[2]){
      
      p_ACME <- ggplot2::ggplot(object$ACME[[cat]], aes(object$ACME[[cat]]$est, stats::reorder(object$ACME[[cat]]$feat, object$ACME[[cat]]$est), color = est <= 0, shape = pval <= 0.05)) +
        geom_vline(xintercept = 0, linetype = "dashed") +
        geom_errorbarh(aes(xmin = object$ACME[[cat]]$CI_2.5, xmax = object$ACME[[cat]]$CI_97.5)) +
        geom_point(size = 2.8) +
        theme_bw() +
        labs(title = "A. ACME (Average Causal Mediation Effect)", y = "Mediators", x="Est. effect [CI 2.5-97.5]") +
        #xlab("ACME (Average Causal Mediation Effect)") +
      ...
      p_PM <- ggplot2::ggplot(object$PM[[cat]], aes(object$PM[[cat]]$est, stats::reorder(object$PM[[cat]]$feat, object$PM[[cat]]$est), color = est <= 0, shape = pval <= 0.05)) +
        geom_vline(xintercept = 0, linetype = "dashed") +
        geom_errorbarh(aes(xmin = object$PM[[cat]]$CI_2.5, xmax = object$PM[[cat]]$CI_97.5)) +
        geom_point(size = 2.8) +
        theme_bw() +
        labs(title = "B. Proportion Mediated (%)", y = "Mediators", x="Est. proportion [CI 2.5-97.5]") +
        # xlab("Proportion Mediated (%)") +
      ...
      df_effect = data.frame(OIE = object$oie_med[[cat]],
                             ODE =  object$ode[[cat]][2],
                             OTE =  object$ote[[cat]][2] )
      df_effect_long <- reshape(df_effect, direction = "long", varying = 1:3, v.names = "value", timevar = "variable", times = c("OIE", "ODE", "OTE"))
      ...
      df = data.frame( mediator = object$xm[[cat]]$feat,
                       step1 = object$xm[[cat]]$Estimate,
                       step2 = object$my[[cat]]$Estimate,
                       ACME = object$ACME[[cat]]$est)
      df = dplyr::arrange(df, dplyr::desc(df$ACME))
      df$mediator <- factor(df$mediator, levels = df$mediator)
      ...
      #plot_final = gridExtra::grid.arrange(p_ACME, p_PM, ncol = 2)
