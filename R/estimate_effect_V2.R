...
estimate_effect_V2 <- function(object , m, boots = 100, sims = 3) {
  ...
  ############# fin boucle not multi
  
 
############# debut boucle multivariate
  if (object$input$multivariate) {
    ...
    X = as.matrix(object$input$X_matrix[,cat])
    X_otherCats = as.data.frame(object$input$X_matrix[,-cat])
    ## -1 colonne (la derniere c'est arbitraire) POUR EVITER LA COLINEARITE
    ...
    if(Y_type == "binary") {
      ...
      mod_direct_effect = glm(Y ~ X + M + covars)
      ode = mod_direct_effect$coefficients
    }
 ...
 ############## fin boucle multi
  ...
  if(object$input$multivariate) {
    obj = list(ACME = ACMEs,
               ...
               ode = odes,
               multivariate = object$input$multivariate,
               X_matrix = object$input$X_matrix
    )
  }
