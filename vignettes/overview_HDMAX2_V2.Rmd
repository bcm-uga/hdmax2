...
Data set is produce from transcription, methylation and patients meta data of PDAC (Pancreatic Ductal AdenoCarcinoma) TCGA data. For this first use case we used binary exposition and continuous outcome, we analyzed 100 samples and 500 potential mediators.
...
$$
y_i = cx_i + \sum_{j=1}^{m} b_jm_{ij}+ \sum_{k=1}^{K} v_k u_{ik}^{(2)} + \epsilon_i
$$
