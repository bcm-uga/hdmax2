...
# ## create list of mediators top10 for each cat exposure

mediators_top10s = list()
for(cat in (1:dim(res_step1$input$X_matrix)[2])){
  mediators_top10s[[cat]] = M[,names(sort(res_step1$max2[[cat]]))[1:10]]
}

res_step2_V2 =  hdmax2::estimate_effect_V2(res_step1,
                                    m = mediators_top10s,
                                    boots = 100,
                                    sims = 100)
...
```{r}
## mediators ACME plots 
hdmax2::plot_V2(res_step2_V2, N_med = 10)
```

```{r}
comp_ACME = cbind(res_step2$ACME[,c(1,5)],res_step2_V2$ACME[[1]][,c(1,5)])
library(knitr)
knitr::kable(comp_ACME)
```
