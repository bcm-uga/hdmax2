...
# for(nX in (1:dim(res_step1$input$X_matrix)[2])){
#   mediators_top10s[[n_med]] = M[,names(sort(res_step1$max2[nX,]))[1:10]]
# }
...
```{r}
## mediators ACME plots 
hdmax2::plot(res_step2_V2, N_med = 10)
```
```{r}
comp_ACME = cbind(res_step2$ACME,res_step2_V2$ACME)
comp_ACME
```
