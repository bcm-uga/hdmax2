...
- the second example with a two variables exposure
...
The **run_AS** function is applied to estimate the effects of exposure $X$ on a matrix $M$ of potential mediators, and the effect of each potential mediators on outcome $Y$. Covariables can be included or not, for the example we decide to include two covariates defined above. 
Exposure is categorial, for the example we choose a vector with character type that will be transformed in factor inside function,  but users can apply transformation to factor if needed (to order categories).
...
```
We obtain three series, for each category of categorial exposure (minus the reference category), of 4-set plots:
...
For this second example we generate an exposure that is data frame with a continuous and a binary variables. 
